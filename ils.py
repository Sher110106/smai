...
def calculate_acceptance_threshold(iteration: int, 
    ...
    # For NON-EUCLIDEAN: ε(t) = 0.01 * 0.1^(t / maxIter)
    progress = iteration / max_iterations
    epsilon = 0.01 * (0.1 ** progress)
    return epsilon
...
def iterated_local_search(initial_tour: List[int],
    ...
    while timer.should_continue() and iteration < max_iterations:
        ...
        # Perturbation phase
        perturbed_tour = perturb_tour(current_tour, problem_type, timer)
        
        # Local search phase
        improved_tour = two_opt_local_search(perturbed_tour, distance_matrix, timer)
...
def multi_start_ils(initial_tours: List[List[int]],
    ...
    for i, initial_tour in enumerate(initial_tours):
        ...
        best_tour = iterated_local_search(initial_tour, distance_matrix, problem_type, timer)
        best_cost = cost(best_tour, distance_matrix)
...
def adaptive_ils(initial_tour: List[int],
    ...
    return iterated_local_search(initial_tour, distance_matrix, problem_type, timer)
